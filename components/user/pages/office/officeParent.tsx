...
const OfficeParent = () => {
  ...
  const properties = useSelector((state: RootState) => state.officeData.offices);
  ...
  <Swiper
    ...
    speed={500}  // Reduced speed for faster transition
    effect="fade"
    className="w-full h-full"
  >
    {property.map((office, index) => (
      <SwiperSlide key={index} className="relative w-full h-screen">
        ...
        >
          ...
          <div className="absolute bottom-6 sm:bottom-20 right-6 sm:right-16 bg-black/60 px-4 sm:px-10 py-3 sm:py-6 rounded-lg shadow-lg text-white border border-white/20 z-20">
...
<p className={`text-sm sm:text-xl text-center mb-6 sm:mb-10 mx-auto text-gray-500 w-full sm:w-2/3 ${montserrat.className}`}>
  Our office spaces provide a professional and dynamic environment tailored to modern businesses, ensuring productivity and success in every square foot.
</p>
